...
if [ -f "$RDKV_SUPP_CONF" ]; then
    ...
    case "$PSK_LINE" in
        *psk=\"*\"*)
            ...
            echo "`/bin/timestamp`:Detected quoted passphrase. Preserving as text." >> /opt/logs/NMMonitor.log
            ;;
        ...
        *psk=[0-9a-fA-F]*)
            ...
            if [ "${#RAW_PSK}" -eq 64 ]; then
                ...
                echo "`/bin/timestamp`:Warning: Unquoted PSK is invalid; expected exactly 64 hexadecimal characters. Treating as no PSK found and continuing." >> /opt/logs/NMMonitor.log
