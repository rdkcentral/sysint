...
if [ -f $pid_file ]; then
    ...
    if [ -d /proc/$pid -a -f /proc/$pid/cmdline ]; then
        ...
        dcmLog "proc entry process name: $processName and running process name `basename $0`"
        if echo "$processName" | grep -q `basename $0`; then
            dcmLog "proc entry cmdline and process name matched."
            if [ "x$ENABLE_MAINTENANCE" == "xtrue" ]; then
                MAINT_DCM_INPROGRESS=13
                eventSender "MaintenanceMGR" $MAINT_DCM_INPROGRESS
            fi
            dcmLog "DCMscript_maintenance already in progress. New instance not allowed "
            exit 0
        fi
    fi
fi
...
dcmLog "Starting execution of DCMscript_maintaince.sh"
...
CURL_INFO="$PERSISTENT_PATH/dcm_curl_info"
rm -rf $CURL_INFO
# Timeout value
timeout=10
CURL_TLS_TIMEOUT=30
...
bootstrap_upload_httplink=$(tr181Set Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.LogUploadEndpoint.URL 2>&1 > /dev/null)
if [ ! -z "$bootstrap_upload_httplink" ]; then
    dcmLog "Overriding the upload url with bootstrap url"
    upload_httplink="$bootstrap_upload_httplink"
fi
...
sendTLSDCMCodebigRequest()
{
    ...
    dcmLog "Curl return code: $TLSRet, http code: $http_code"

    case $TLSRet in
        35|51|53|54|58|59|60|64|66|77|80|82|83|90|91)
            tlsLog "CERTERR, MAINT_DCM, $TLSRet, $FQDN"
            t2ValNotify "certerr_split" "MAINT_DCM, $TLSRet, $FQDN"
            ;;
    esac
}
...
sendHttpRequestToServer()
{
    ...
    if [ $UseCodebig -eq 1 ]; then
        ...
        dcmLog "Check if codebig is supported for the Device"
        checkCodebigAccess
        codebigapplicable=$?
...
scheduleSupplementaryServices() {
    ...
    if [ -n "$upload_protocol" ]; then
        ...
        upload_protocol='HTTP'
        dcmLog "'urn:settings:LogUploadSettings:Protocol' is not found in DCMSettings.conf, upload_protocol is '$upload_protocol'"
    fi
    ...
    if [ "$mTlsLogUpload" == "true" ] || [ $useXpkiMtlsLogupload == "true" ]; then
        ...
        if [ "$FORCE_MTLS" != "true"  ] && [ -n "$httplink" ]; then
...
{
     dcmLog "DCM is interrupted due to the maintenance abort"
...
then
   ...
   then
        dcmLog "Sending MAINT_DCM_ERROR event to MaintenanceMGR"
        eventSender "MaintenanceMGR" $MAINT_DCM_ERROR
   else
        dcmLog "Sending MAINT_DCM_COMPLETE event to MaintenanceMGR"
