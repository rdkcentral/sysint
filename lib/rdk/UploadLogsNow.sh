...
modifyFileWithTimestamp()
{
    ...
    do
        ...
        if [ $test1 -gt 0 -o $test2 -gt 0 -o $test3 -gt 0 -o $test4 -gt 0 ];  then
            echo "`/bin/timestamp` Processing file...$f" | systemd-cat -t UploadLogsNow
        else
            mv $f $dt$f
        fi
    done
...
HttpLogUpload()
{
    ...
    echo "`/bin/timestamp` MTLS defaulted" | systemd-cat -t UploadLogsNow
    msg_tls_source="mTLS certificate from xPKI"
    echo "`/bin/timestamp` Connect with $msg_tls_source" | systemd-cat -t UploadLogsNow
    echo "`/bin/timestamp` URL_CMD: $CURL_CMD" | systemd-cat -t UploadLogsNow
	...
	ret=` exec_curl_mtls "$CURL_CMD" "echo"`
        ...
        echo "`/bin/timestamp` Retrying SNMP based HTTP log upload - $retries" | systemd-cat -t UploadLogsNow
        sleep 1
    done
    
    if [ $http_code -eq 200 ];then
        echo "`/bin/timestamp` S3 upload query success. Got new S3 url to upload log" | systemd-cat -t UploadLogsNow
	...
	ret=` exec_curl_mtls "$CURL_CMD" "echo"`
        ...
        if [ $http_code -eq 200 ];then
            result=0
			echo "`/bin/timestamp` Uploaded Logs through - SNMP/TR69" | systemd-cat -t UploadLogsNow
            echo "Complete $(date)"   > /opt/loguploadstatus.txt

        else
            echo "`/bin/timestamp` Failed Uploading Logs through - SNMP/TR69" | systemd-cat -t UploadLogsNow
			echo "Failed $(date)" > /opt/loguploadstatus.txt

        fi
    else
        echo "`/bin/timestamp` S3 upload query Failed" | systemd-cat -t UploadLogsNow
        echo "Failed $(date)" > /opt/loguploadstatus.txt
...
echo "`/bin/timestamp` Uploading Logs through SNMP/TR69 Upload" | systemd-cat -t UploadLogsNow
modifyFileWithTimestamp $DCM_LOG_PATH | systemd-cat -t UploadLogsNow 2>&1
tar -zcvf $LOG_FILE * | systemd-cat -t UploadLogsNow 2>&1
