...
# Read monotonic uptime in seconds (integer portion)
get_uptime_s() {
...
SCRIPT_START_S=$(get_uptime_s)
backupLog "backup_logs: script started"
...
backupLog "disk_threshold_check: started"
    DISK_START_S=$(get_uptime_s)
    sh /lib/rdk/disk_threshold_check.sh 0
    DISK_END_S=$(get_uptime_s)
    backupLog "disk_threshold_check: completed (exit=$?) elapsed=$((DISK_END_S - DISK_START_S))s"
fi
...
backupLog "systemd-notify: started"
NOTIFY_START_S=$(get_uptime_s)
/bin/systemd-notify --ready --status="Logs Backup Done..!"
NOTIFY_END_S=$(get_uptime_s)
backupLog "systemd-notify: completed (exit=$?) elapsed=$((NOTIFY_END_S - NOTIFY_START_S))s"

SCRIPT_END_S=$(get_uptime_s)
backupLog "backup_logs: script completed elapsed=$((SCRIPT_END_S - SCRIPT_START_S))s"
